...
public class TvDaoImpl implements TvDao {
    ...
    @Override
    public int UpdateDataByFields(int id, String name, String channel, double trp, int ratings, String language) {
        Connection connection = null;
        PreparedStatement statement = null;
        try {
            connection = DriverManager.getConnection(URL, USER, PASS);
            connection.setAutoCommit(false);
            statement = connection.prepareStatement("update serials set serials_channel=?, serials_trp=?, serial_rating=?, serial_language=? where id=? AND serial_name=?");

            System.out.println("Updating record with ID: " + id + " and Name: " + name);
            System.out.println("New values - Channel: " + channel + ", TRP: " + trp + ", Ratings: " + ratings + ", Language: " + language);
